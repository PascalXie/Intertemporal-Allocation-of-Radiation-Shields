...
if __name__ == '__main__':
	...
	#
	path = '../step5-integerLinearProgramming/'
	...
	for i in range(4):
		num = 12 + 12*i
		fileName = 'IntertemporalAllocationResults_ILP_'+str(num)+'.txt'
	...
	#plt.title('Intertemporal Allocation of Shields with ILP')
	##plt.xlim(-0.5,11)
	##plt.ylim(0,2500)
	#plt.tight_layout()
	#plt.savefig('figure-level6_allocationShields_ILP.png',dpi=300)
	...
	for i in range(4):
		num = 12 + 12*i
		Doses_t1 = Doses_All_t1[i]
		Doses_t2 = Doses_All_t2[i]
	...
	#plt.title('Absorption Doses in Two Periods with ILP')
	##plt.xlim(-0.5,11)
	##plt.ylim(0,2500)
	#plt.tight_layout()
	#plt.savefig('figure-level6_dosesInTwoPeriods_ILP.png',dpi=300)

	#
	# plot 3
	#
	fo = open("MalfunctionRatio_ILP.txt","w")
